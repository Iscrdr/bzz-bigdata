 ...
 * @version: 1.0.0
 */
public class CourseCountApp {
	public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
		Configuration conf = new Configuration();
		Job job = Job.getInstance(conf, "CourseCountApp");
		job.setJarByClass(CourseCountApp.class);
		
		job.setMapperClass(CourseCountMapper.class);
		job.setMapOutputKeyClass(Text.class);
		job.setMapOutputValueClass(Text.class);
		
		job.setReducerClass(CourseCountReducer.class);
		job.setOutputKeyClass(Text.class);
		job.setOutputValueClass(Text.class);
		
		String outPath = "D:\\code\\bzz-bigdata\\bzz-mapreduce\\case\\case6\\output";
